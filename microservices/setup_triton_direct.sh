...
if [ -d "${MICROSERVICES_DIR}/tritonserver" ] && [ -f "${MICROSERVICES_DIR}/tritonserver/bin/tritonserver" ]; then
    ...
    if [ "$DOWNLOAD_SUCCESS" = false ]; then
        ...
        echo ""
        echo "Option 2: Manual download commands"
        echo "  cd ${MICROSERVICES_DIR}"
        echo "  # Try version 2.47.0 (Ubuntu 22.04 - compatible with 24.04):"
        ...
        echo "  rm tritonserver-2.47.0-ubuntu22.04.tar.gz"
        echo ""
        echo "Option 3: Visit releases page"
        echo "  https://github.com/triton-inference-server/server/releases"
        echo "  Look for 'ubuntu22.04' or 'ubuntu20.04' packages (compatible with 24.04)"
        echo ""
        exit 1
    fi
