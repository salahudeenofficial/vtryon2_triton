...
if [ -d "${MICROSERVICES_DIR}/tritonserver" ] && [ -f "${MICROSERVICES_DIR}/tritonserver/bin/tritonserver" ]; then
    ...
    if [ "$DOWNLOAD_SUCCESS" = false ]; then
        ...
        echo ""
        echo "=========================================="
        echo "Alternative: Install via PyTriton (Recommended)"
        echo "=========================================="
        echo ""
        echo "PyTriton includes Triton Inference Server binaries and works on Ubuntu 24.04"
        echo ""
        echo "Run:"
        echo "  cd ${MICROSERVICES_DIR}"
        echo "  chmod +x install_triton_via_pytriton.sh"
        echo "  ./install_triton_via_pytriton.sh"
        echo ""
        echo "This will:"
        echo "  - Install nvidia-pytriton (includes Triton server)"
        echo "  - Set up Triton binary in expected location"
        echo "  - Install Python backend dependencies"
        echo ""
        echo "=========================================="
        echo "Or Try Manual Download"
        echo "=========================================="
        echo ""
        echo "Option 1: Use manual install script"
        echo "  ./manual_triton_install.sh"
        echo ""
        echo "Option 2: Visit releases page and find working version"
